...
public class MenuListPage extends AbstractPage {
	...
	{			
		...
		for(int i = 0; i < menuData.length; i+=2)
			SetButton(a, menuData[i].name, Ids.BTN_MENU_LIST_START + i, btnR1X, btnY + btnYAdjust * (i / 2), this.menuButton_width, this.menuButton_height);

		for(int i = 1; i < menuData.length; i+=2)
			SetButton(a, menuData[i].name, Ids.BTN_MENU_LIST_START + i, btnR2X, btnY + btnYAdjust * (i / 2) , this.menuButton_width, this.menuButton_height);
