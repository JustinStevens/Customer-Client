...
public class MenuListPage extends AbstractPage {
	...
	public MenuListPage (MainActivity a, int id, int pWidth, int pHeight, String[] menuNames)
	{			
		...
		for(int i = 0; i < menuNames.length; i+=2)
			SetButton(a, menuNames[i], Ids.BTN_MENU_LIST_START + i, btnR1X, btnY + btnYAdjust * (i / 2), this.menuButton_width, this.menuButton_height);
		
		for(int i = 1; i < menuNames.length; i+=2)
			SetButton(a, menuNames[i], Ids.BTN_MENU_LIST_START + i, btnR2X, btnY + btnYAdjust * (i / 2) , this.menuButton_width, this.menuButton_height);
